...
function ign_menu( $item, $args ) {
	...
	if ( in_array( 'menu-item-has-children', (array) $classes ) ) {
		$item .= '<button tabindex="-1" class="submenu-dropdown-toggle">' . $arrow . '
                    <span class="screen-reader-text">' . __( 'Expand child menu', 'lifted' ) . '</span></button>';
	}
 ...
 * @return void
 */
function lifted_output_inline_svg( $html ) {
...
add_filter( 'get_custom_logo', 'lifted_output_inline_svg' );
 ...
 *
 */
function lifted_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'lifted-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		$urls[] = array(
...
add_filter( 'wp_resource_hints', 'lifted_resource_hints', 10, 2 );
